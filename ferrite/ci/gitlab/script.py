...
class Job:
    ...
    def text(self, cache: Optional[Cache]) -> List[str]:
        ...
        artifacts = [art for art in self.artifacts() if cache is None or not cache.is_cached(art)]
        if len(artifacts) > 0:
            lines.extend([
                "  artifacts:",
                "    paths:",
                *[f"      - {base_path(art)}" for art in artifacts],
            ])
...
def generate(end_tasks: List[str], cache: Optional[Cache]) -> str:
    graph = Graph()
